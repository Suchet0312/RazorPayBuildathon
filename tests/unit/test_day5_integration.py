...
def test_day5_end_to_end_integration():
    """
    Day 3 -> Day 4 -> Day 5 vertical slice.
    ...
    # Build the exact Day 3 ML feature set.
    ...
    # Day 5: deterministic policy decision.
    ...
    # Verify the pipeline reached every stage.
    ...
    # Verify Day 4 expected recovery value.
    assert recovery_plan.expected_recovery_value == round(
        payment.amount * recovery_probability,
        2,
    )

    # Verify Day 5 always produces a valid explicit decision.
    assert isinstance(policy_decision.approved, bool)
    assert policy_decision.reason
